...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.ft = FeatureTransform(num_transforms=1)
        self.do_ft = True

    def forward(self, encoder_hidden_states):

        if self.do_ft:
            encoder_hidden_states = self.ft.transform_features(encoder_hidden_states)

        x1, x2, x3, x4 = encoder_hidden_states
        out1, out2 = self.dcfam(x1, x2, x3, x4)
        x = out1 + self.up(out2)
        x = self.dropout(x)

        if self.do_ft:
            x = self.ft.invert_transformation(x)

        x = self.segmentation_head(x)

        if self.do_ft:
            logits_original, logits_transformed = self.ft.split_logits(x)
            return logits_original, logits_transformed
...
def dcswin_base(pretrained=True, num_classes=4, weight_path='models/decoders/dcfam/pretrained_weights/stseg_base.pth'):
    ...
    if pretrained and weight_path is not None:
        ...
        # print('weights: ', len(old_dict))
        old_dict = {k: v for k, v in old_dict.items() if (k in model_dict)}
        # print('updated: ', len(old_dict))
        model_dict.update(old_dict)
        model.load_state_dict(model_dict)
    return model


def dcswin_small(pretrained=True, num_classes=4, weight_path='models/decoders/dcfam/pretrained_weights/stseg_small.pth'):
...
def dcswin_tiny(pretrained=True, num_classes=4, weight_path='models/decoders/dcfam/pretrained_weights/stseg_tiny.pth'):
    model = DCSwin(encoder_channels=(96, 192, 384, 768),
                   num_classes=num_classes,
                   embed_dim=96,
                   depths=(2, 2, 6, 2),
